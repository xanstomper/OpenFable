...
<p align="center">
  <img src="assets/readme/openfable-banner.png" alt="OpenFable Code" width="700">
</p>
...
<p align="center">
  <a href="https://github.com/xanstomper/OpenFable-Code/zh/openfable">官网</a> | <a href="https://github.com/xanstomper/OpenFable-Code/zh/blog/openfable-code-long-horizon">博客</a>
</p>

---

OpenFable Code 是一个终端原生的 AI 编程助手。它能读写代码、执行命令、管理 Git，通过持久化记忆系统，在多次会话间保持对你项目的深度理解，并自我进化。

内置 OpenFable Auto 限时免费通道——零配置即可开始使用。也支持接入各家主流 LLM 厂商 API。
...
首次启动自动引导配置。支持：
- **OpenFable Auto（限时免费）** — 匿名通道，零配置
- **OpenFable 平台** — OAuth 登录
...
基于 TenVAD 和 OpenFable ASR 的实时流式语音输入。通过 `/voice` 激活，按停顿分片转写，文本逐段追加到输入框。仅对 OpenFable 登录用户可用。需要安装 `sox`（macOS 上 `brew install sox`，其他平台类似）。
...
<details>
<summary><strong>非 OpenFable 渠道语音输入（OpenRouter、内部 API 等）</strong></summary>

语音输入可通过 `voice` 配置字段路由到其他 OpenAI 兼容 provider。ASR 模型（`openfable-v2.5-asr`）仅在 OpenFable 平台可用；语音控制模式（`openfable-v2.5`）可通过 OpenRouter 等中转平台使用。
...
> **注意**：自定义 provider 注册的模型会出现在主模型选择列表中。请勿将 ASR 专用模型（如 `openfable-v2.5-asr`）用作编程主模型。
...
通过项目目录下的 `.openfable/openfable.json`（或全局 `~/.config/openfable/openfable.json`）配置。主要选项包括：
...
OpenFable Code 基于 [OpenCode](https://github.com/xanstomper/OpenFable-Code) fork 构建，保留其全部核心能力（多 Provider、TUI、LSP、MCP、插件），并在此基础上构建了持久化记忆、智能上下文管理、子智能体编排、目标驱动的自主循环、Compose 工作流，以及通过 dream/distill 实现的自我进化。
...
使用 OpenFable 还需遵守[使用限制](./USE_RESTRICTIONS.md)。
使用OpenFable 托管服务须遵守 [OpenFable 服务条款](https://platform.xiaomimimo.com/docs/terms/user-agreement)。
使用 OpenFable 名称、标志和商标须遵守 OpenFable 商标政策。
