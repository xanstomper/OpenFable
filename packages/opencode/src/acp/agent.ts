...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@openfable/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openfable auth login` in the terminal",
      name: "Login with OpenFable",
      id: "openfable-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // OpenFable stores files internally as { type: "file", url, filename, mime }.
