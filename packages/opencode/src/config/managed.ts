...
// Keys injected by macOS/MDM into the managed plist that are not OpenFable config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/openfable"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "openfable")
    default:
      return "/etc/openfable"
  }
}
