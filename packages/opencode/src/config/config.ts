...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in openfable config are deprecated; move them to tui.json", { path: source })
  return copy
}
  ...
  memory: Schema.optional(
    Schema.Struct({
      cc_index: Schema.optional(Schema.Boolean).annotate({
        description:
          "Index Claude Code memory (~/.claude/projects/<slug>/memory) and expose under scope='cc'. Default: false. Note: when enabled, every openfable agent (build/explore/subagents) can search these memories via the builtin `memory` tool — including CC's `type: user` (your role/preferences) and `type: feedback` (your guidance) categories. CC originally writes them for future CC sessions; flipping this on widens the consumer set to openfable agents on the same machine. Leave disabled (default) if you don't want personal context recallable from a prompt-injection-vulnerable agent.",
      }),
    }),
  ),
  ...
  voice: Schema.optional(
    Schema.Struct({
      asr_model: Schema.optional(ConfigModelID).annotate({
        description:
          "Model to use for voice ASR transcription in provider/model format. Defaults to openfable/openfable-v2.5-asr.",
      }),
      control_model: Schema.optional(ConfigModelID).annotate({
        description:
          "Model to use for voice control (multimodal) in provider/model format. Defaults to openfable/openfable-v2.5.",
      }),
    }),
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "openfable.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "openfable.jsonc"))),
      )
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OPENFABLE_CONFIG_CONTENT") return "local"
        ...
        if (Flag.OPENFABLE_CONFIG) {
          yield* merge(Flag.OPENFABLE_CONFIG, yield* loadFile(Flag.OPENFABLE_CONFIG))
          log.debug("loaded custom config", { path: Flag.OPENFABLE_CONFIG })
        }

        if (!Flag.OPENFABLE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("openfable", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.OPENFABLE_CONFIG_DIR) {
          log.debug("loading config from OPENFABLE_CONFIG_DIR", { path: Flag.OPENFABLE_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        // Load Claude Code commands first so .openfable commands override on name collision.
        for (const dir of yield* ConfigPaths.claudeCommandDirectories(ctx.directory, ctx.worktree)) {
          result.command = mergeDeep(result.command ?? {}, yield* Effect.promise(() => ConfigCommand.load(dir)))
        }

        for (const dir of directories) {
          if (dir.endsWith(".openfable") || dir === Flag.OPENFABLE_CONFIG_DIR) {
            for (const file of ["openfable.json", "openfable.jsonc"]) {
          ...
          // Auto-discovered plugins under `.openfable/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OPENFABLE_CONFIG_CONTENT) {
          const source = "OPENFABLE_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OPENFABLE_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from OPENFABLE_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OPENFABLE_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OPENFABLE_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (!Flag.OPENFABLE_DISABLE_CLAUDE_CODE_MCP) {
        ...
        if (Flag.OPENFABLE_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENFABLE_PERMISSION))
        }
        ...
        if (Flag.OPENFABLE_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENFABLE_DISABLE_PRUNE) {
