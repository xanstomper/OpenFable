...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run openfable",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for openfable session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found openfable session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["openfable", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting openfable...")
        UI.println()

        const openfableArgs = sessionId ? ["-s", sessionId] : []
        const openfableProcess = Process.spawn(["openfable", ...openfableArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await openfableProcess.exited
        if (code !== 0) throw new Error(`openfable exited with code ${code}`)
      },
    })
  },
})
