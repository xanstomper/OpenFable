...
import { OPENFABLE_PROCESS_ROLE, OPENFABLE_RUN_ID, ensureRunID, sanitizedProcessEnv } from "@/util/openfable-process"
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start openfable tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start openfable in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const env = sanitizedProcessEnv({
        [OPENFABLE_PROCESS_ROLE]: "worker",
        [OPENFABLE_RUN_ID]: ensureRunID(),
      })
