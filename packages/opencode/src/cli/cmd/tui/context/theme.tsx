...
import openfable from "./theme/openfable.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@openfable/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "openfable")
        draft.active = typeof active === "string" ? active : "openfable"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.openfable, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  return {
    theme: {
      ...
      diffRemovedBg,
      diffContextBg,
      diffLineNumber: grays[4],
      diffAddedLineNumberBg: tint(diffContextBg, ansiColors.green, diffAlpha * 0.4),
      diffRemovedLineNumberBg: tint(diffContextBg, ansiColors.red, diffAlpha * 0.4),

      // Markdown colors - vibrant and readable
      markdownText: fg,
      markdownHeading: openfableBlue,
      markdownLink: openfableCyan,
      markdownLinkText: openfableCyan,
      markdownCode: openfableGreen,
      markdownBlockQuote: openfableAmber,
      markdownEmph: openfableAmber,
      markdownStrong: fg,
      markdownHorizontalRule: grays[5],
      markdownListItem: openfableBlue,
      markdownListEnumeration: openfableCyan,
      markdownImage: openfableBlue,
      markdownImageText: openfableCyan,
      markdownCodeBlock: fg,

      // Syntax colors - using OpenFable accent palette
      syntaxComment: textMuted,
      syntaxKeyword: ansiColors.magenta,
      syntaxFunction: openfableBlue,
      syntaxVariable: fg,
      syntaxString: openfableGreen,
      syntaxNumber: openfableAmber,
      syntaxType: openfableCyan,
      syntaxOperator: openfableCyan,
