...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@openfable/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run openfable with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openfable server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.OPENFABLE_AUTO_SHARE && !args.share) return
