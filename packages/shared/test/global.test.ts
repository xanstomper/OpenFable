...
describe("resolveOpenfableHome", () => {
  test("with OPENFABLE_HOME set, resolves 4 subdirs under root", () => {
    const result = resolveOpenfableHome({
      OPENFABLE_HOME: "/tmp/profile-a",
    })
    expect(result.mode).toBe("openfable_home")
  ...
  test("empty OPENFABLE_HOME string is treated as unset (xdg mode)", () => {
    const result = resolveOpenfableHome({ OPENFABLE_HOME: "" })
    expect(result.mode).toBe("xdg")
  })

  test("relative OPENFABLE_HOME path throws with clear error", () => {
    expect(() => resolveOpenfableHome({ OPENFABLE_HOME: "./foo" })).toThrow(
      /OPENFABLE_HOME must be an absolute path/,
    )
    expect(() => resolveOpenfableHome({ OPENFABLE_HOME: "foo/bar" })).toThrow(
      /OPENFABLE_HOME must be an absolute path/,
    )
  })

  test("tilde-prefixed OPENFABLE_HOME throws (not treated as absolute)", () => {
    expect(() => resolveOpenfableHome({ OPENFABLE_HOME: "~/profiles/a" })).toThrow(
      /OPENFABLE_HOME must be an absolute path/,
    )
  })

  test("error message includes the offending value", () => {
    expect(() => resolveOpenfableHome({ OPENFABLE_HOME: "./relative" })).toThrow(
      /\.\/relative/,
    )
  })
})
